"""
Port Physical Constraint and Vessel Compatibility Checking Engine.
Enforces hard physical constraints (draft, LOA, beam, DWT, vessel class) and generates explicit exclusion reason codes.
"""
import logging
...
def check_vessel_port_compatibility(
    ...
    """
    Evaluate hard physical compatibility between a vessel candidate and a destination port.
    ...
    if not constraints:
        reasons.append(f"Port '{port_name}' not found in canonical registry.")
...
def filter_feasible_vessels(
    ...
    """
    Filter candidate fleet into (feasible_df, excluded_df) with audit trails.
    """
    feasible_records = []
    excluded_records = []
